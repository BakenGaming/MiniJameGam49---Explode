...
public class EnemyHandler : MonoBehaviour, IEnemyHandler, IDamageable
{
    ...
    public void TakeDamage(int _damage)
    {
        ...
        if(_healthSystem.GetCurrentHealth() == 0)
        {
            ...
            if(dropTwo)
            {
                CollectableObjectHandler bonusCollectable = ObjectPooler.DequeueObject<CollectableObjectHandler>("Collectable");
                bonusCollectable.transform.position = transform.position;
                bonusCollectable.transform.rotation = transform.rotation;
                bonusCollectable.gameObject.SetActive(true);
                bonusCollectable.InitializeCollectable(enemySO.lootBag[1]);
            }
    ...
    {
        if (Random.Range(0f,100f) >= 75f) return true;
        else return false;
    }
    #endregion
    #region Enemy Setup
